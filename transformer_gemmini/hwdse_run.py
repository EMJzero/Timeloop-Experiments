...
hw_configs = [
    # 3 designs varying in SA rows/cols
    {
        ...
        'accumulator_size': 16384, # multipled by pe_rows in NOs mode
        'scratchpad_bandwidth': 16,
        'accumulator_bandwidth': 4,
        # matmul only
        'pe_rows': 16,
        'pe_cols': 16,
        'dataflow': "WS",
        ...
        # shared
        'scratchpad_size': 262144*2,
        'accumulator_size': 16384*2, # multipled by pe_rows in NOs mode
        'scratchpad_bandwidth': 16,
        'accumulator_bandwidth': 4,
        # matmul only
        'pe_rows': 8,
    ...
    # same 3 designs as above, with double on-chip memory and bandwidth
    {
        ...
        'accumulator_size': 16384*2, # multipled by pe_rows in NOs mode
        'scratchpad_bandwidth': 16*2,
        'accumulator_bandwidth': 4*2,
        # matmul only
        'pe_rows': 16,
        'pe_cols': 16,
        'dataflow': "WS",
        ...
        # shared
        'scratchpad_size': 262144*4,
        'accumulator_size': 16384*4, # multipled by pe_rows in NOs mode
        'scratchpad_bandwidth': 16*2,
        'accumulator_bandwidth': 4*2,
        # matmul only
        'pe_rows': 8,
...
# Note: sort_key must be a lambda extracting the attribute to sort on
def summary(results, msg = "RESULTS SUMMARY:", print_tested = False, sort_key = lambda x : x['metrics']['edp']):
    print(f"\n\n{msg}\n")
    if print_tested:
...
for hw_config in (hw_configs if not options['mixup'] else mixup_dicts(hw_configs)):
    ...
    for layer in target_ops:
        ...
        OS_row_spatial_constr_factors = tl.constraints.Factors(["L=1", "E=1", f"D>={hw_config['pe_rows']//2}"])
        OS_col_spatial_constr_factors = tl.constraints.Factors(["E=1", "D=1", f"L>={hw_config['pe_cols']//2}"])
        ...
        buf.attributes["read_bandwidth"] = hw_config['scratchpad_bandwidth']
        buf.attributes["write_bandwidth"] = hw_config['scratchpad_bandwidth']
        ...
        acc.attributes["read_bandwidth"] = hw_config['accumulator_bandwidth'] * (1 if not is_norm_op else hw_config['pe_rows'])
        acc.attributes["write_bandwidth"] = hw_config['accumulator_bandwidth'] * (1 if not is_norm_op else hw_config['pe_rows'])
        if not is_norm_op:
            ...
            for constr in spec.constraints['targets']:
                # Fix maximize_dims
                if constr['target'] == "Accumulator" and 'maximize_dims_capacity' in constr:
                    constr['maximize_dims_capacity'] = acc.attributes["entries"] // hw_config['rf_size']
