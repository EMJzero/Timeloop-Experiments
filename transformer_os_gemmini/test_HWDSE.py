...
def run_test(global_buffer_size_scale: float, pe_scale_X: float, pe_scale_Y: float, brief_print: bool=False):
    if brief_print:
        print('.', end='')
    ...
    pe.spatial.meshX = round(pe.spatial.meshX * pe_scale_X)
    pe.spatial.meshY = round(pe.spatial.meshY * pe_scale_Y)
    spec.mapper.search_size = 2000

    # Give each run a unique ID and run the mapper
    proc_id = f"glb_scale={global_buffer_size_scale},pe_scale_X={pe_scale_X},pe_scale_Y={pe_scale_Y}"
    if brief_print:
        print('.', end='')
    else:
    ...
    return (
        ...
        spec.architecture.find("PE").spatial.meshY,
        energy,
    )
...
for global_buffer_size_scale in [0.5, 1, 2]:
    for n_pes_X in [0.5, 1, 2]:
        for n_pes_Y in [0.5, 1, 2]:
            arg = (global_buffer_size_scale, n_pes_X, n_pes_Y)
            args.append(arg)
...
for global_buffer_depth, n_pes_X, n_pes_Y, energy in results:
    print(
        f"Global buffer depth: {global_buffer_depth}, PEsX: {n_pes_X}, PEsY: {n_pes_Y}, pJ/MAC: {energy}"
    )
